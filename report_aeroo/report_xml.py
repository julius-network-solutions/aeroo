...
class report_xml(orm.Model):
    ...
    def _report_content(self, cursor, user, ids, name, arg, context=None):
        res = {}
        aeroo_ids = self.search(cursor, SUPERUSER_ID, [('report_type','=','aeroo'),('id','in',ids)], context=context)
        orig_ids = list(set(ids).difference(aeroo_ids))
        res = orig_ids and super(report_xml, self)._report_content(cursor, SUPERUSER_ID, orig_ids, name, arg, context) or {}
        for report in self.read(cursor, SUPERUSER_ID, aeroo_ids, ['tml_source','report_type','report_sxw_content_data', 'report_sxw'], context=context):
...
#            self._set_report_wizard(cr, user, res_id, context)
    ...
    def _set_report_wizard(self, cr, uid, ids, report_action_id, linked_report_id=False, report_name=False, context=None):
        ...
        if event_id:
            ...
            translations = trans_obj.search(cr, uid, [('res_id','=',id),('src','=',report.name),('name','=','ir.actions.report.xml,name')])
