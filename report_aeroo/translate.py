...
def extend_trans_generate(lang, modules, cr):
    ...
    root_path = os.path.join(config['root_path'], 'addons')

    apaths = map(os.path.abspath, map(str.strip, config['addons_path'].split(',')))
    ...
    for bin_path in ['osv', 'report' ]:
        path_list.append(os.path.join(config['root_path'], bin_path))
